...
| **ADR-136** | Worker binding types are generated by `wrangler types`, never hand-written | Accepted | `apps/api/worker-configuration.d.ts` (committed; regenerated by `pnpm types`, which the typecheck script runs first) supplies the `Env` interface plus runtime types pinned to the configured compatibility date, replacing both the hand-written binding type and the `@cloudflare/workers-types` dependency — configuration and code cannot drift. Generated with `--strict-vars=false`: vars stay `string`-typed because one codebase serves both environments; runtime Zod validation (`src/env.ts`) owns correctness. |
