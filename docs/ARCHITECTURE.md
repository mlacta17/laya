...
*Private streaming platform · v1.3.13 D1 baseline · August 6, 2026*
...
13. **Encoded-storage multiplier and retention levers:** Bunny's current documentation and the disposable-library dashboard confirm that original retention and MP4 fallback are per-library settings applied to subsequent uploads. In Phase 0B, `movie-01` stored 8.17× its source bytes (≈4.7 GB per content hour), while a deliberately non-representative 10:34 synthetic control stored 6.78× (≈10.6 GB per content hour) because it produced a full 1080p ladder from a different source profile. The retained originals and MP4 fallback materially increase both totals. Do not average these incompatible samples or replace §11.2's planning baseline with the short control. Before Phase 1 Bunny integration: measure a representative episode, choose and record the production retention/fallback policy (offline downloads in Phase 5 are expected to depend on MP4 fallback; originals already live on the owner's drive under ADR-110), and then re-baseline §11.2 from the representative movie/episode evidence.
...
| v1.3.13 | Aug 6, 2026 | Bunny storage-setting evidence added. Official documentation and the disposable dashboard confirm that original retention and MP4 fallback are per-library, future-upload settings. A completed 10:34 synthetic control is retained as useful storage evidence but explicitly rejected as the representative episode, preventing an invalid §11.2 cost rebaseline. |
