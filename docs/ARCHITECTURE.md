...
12. **JWKS refresh cooldown (Phase 0B, with the real provider):** an RS256 token naming an unknown `kid` forces a JWKS refetch (`apps/api/src/auth/verify-token.ts`). Concurrent requests share one fetch, but nothing limits how often sequential garbage tokens can trigger refetches against the provider. Add a minimum interval between forced refreshes — the pattern mature JWKS caches use — when the provider integration lands; irrelevant while dev uses inline mock keys.
