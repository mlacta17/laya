...
| **ADR-134** | Dev Worker serves the mock-issuer JWKS at a dev-only route; production deploys from 0A with a no-issuer auth config | Accepted | The deployed dev Worker hosts the mock issuer's public JWKS at a dev-only route and dev configuration points the verifier's JWKS URL at that route, so deployed-dev verification exercises the same JWKS fetch/cache path as local development (required by the Phase 0A rejection matrix). Production deploys from Phase 0A with no issuer configured at all: **every authenticated request in production returns 401 until Phase 0B selects the real provider** — this is intended, and it makes the ADR-132 failing-request acceptance test real. Production configuration contains neither the JWKS route nor any mock audience, JWKS URL or key (ADR-132). |
