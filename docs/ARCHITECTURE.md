...
*Private streaming platform · v1.3.15 D1 baseline · August 9, 2026*
...
- Errors are classified as retryable or terminal in the shared contract. The code vocabulary is `invalid_request`, `unauthorized`, `forbidden`, `not_found`, `payload_too_large`, `internal`; only `internal` is retryable. `unauthorized` (401) means authentication failed; `forbidden` (403) means a verified identity lacks Laya permission (ADR-144). Where acknowledging a resource's existence would leak information, respond `not_found` instead.
...
| **ADR-144** | `forbidden` (403) joins the shared error contract for authenticated permission denials | Accepted | Accepts the convention the Phase 0B invitation/revocation spike proposed (§7) and `auth-membership-boundary.test.ts` models: `unauthorized` (401) means authentication failed — no identity was established; `forbidden` (403) means a cryptographically verified identity lacks Laya permission (no active library membership or missing role). `forbidden` is terminal, never retryable: the caller cannot succeed until an operator changes their membership. Responses carry the generic envelope message only; membership details, roles and other identities stay in structured logs and the audit trail. Where acknowledging that a resource exists would itself leak information — unknown invitation tokens, operator-only routes probed by non-operators — routes respond `not_found` instead of `forbidden`. Phase 1 authorization middleware must use this code for steps 3–4 of the §7 authorization order. |
...
| v1.3.15 | Aug 9, 2026 | Pre-Phase-1 contract hardening from the post-0B code review. ADR-144 accepts the spike-proposed `forbidden` (403) error code: authenticated identity without Laya permission, terminal and non-retryable, with `not_found` retained where acknowledging existence would leak; §3.3 now enumerates the full error-code vocabulary and the 401/403 boundary, and the membership boundary test adopts the shared contract instead of its local Phase 0B model. Worker logging is consolidated behind a typed structured-log helper (`apps/api/src/log.ts`) with a standard thrown-value serialization, so Phase 1's request/provider-transition logging extends one documented envelope instead of repeating ad-hoc `console` literals. |
