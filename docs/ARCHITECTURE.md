...
| Authentication | Clerk managed authentication; provider-neutral RS256/JWKS verification in the Worker | Phase 0B passed invite-only browser and Expo flows, passwordless email, Google, Apple, revocation and Worker-side token verification; production SDK/config integration begins in Phase 1 |
...
### 3.3 API module boundaries and conventions
...
- Browser access uses an exact, environment-validated web-origin CORS
  allowlist; wildcard origins are forbidden for authenticated API routes.
...
**S3 · Manual** — operator relabel, replace, set default/forced/SDH flags, or delete. ADR-140's caption-scoped zero-cache rule supports replacement and deletion; normal mutations do not purge the entire pull zone.
...
- Clerk is the single accepted managed auth provider (ADR-127); Auth0 remains a rejected spike candidate, not a simultaneous production dependency.
- Phase 0B passed passwordless email, Google, Apple, browser sessions, physical-iPhone Expo sessions, logout, revocation and Worker-side JWT verification. A trusted real-Philippines run remains a pre-production gate.
- The Worker validates Clerk token signature, issuer, audience and expiry against Clerk's JWKS metadata. Web and mobile obtain bearer tokens through their platform SDKs; neither client receives a provider secret.
...
- ADR-137 needs no cookie-CORS amendment: Laya continues sending explicit bearer tokens in `Authorization`, and authenticated API CORS remains restricted to the exact configured web origin.
...
| **ADR-115** | Production-grade delivery for passwordless and invitation emails | Accepted; amended | Start with Clerk's production-instance email delivery using Laya's production domain and required SPF/DKIM records. Development `accounts.dev` delivery is not acceptable for external users. Add a dedicated transactional provider only if measured deliverability, template/control requirements or provider limits justify another dependency. |
...
| **ADR-127** | Clerk is the managed OIDC/JWT authentication provider | Accepted | Phase 0B proved Auth0 and Clerk across React, physical-iPhone Expo, passwordless email, Google, Apple, reopen/refresh/logout, provider revocation, Worker JWT/JWKS verification, invitation enforcement, recovery, audit and US reliability. Clerk wins at Laya's scale: its Hobby plan is USD 0/month at 10 and 100 users and includes a documented production email path, while Auth0 needs Essentials at USD 35/month for separate environments plus an external production email provider. Clerk's fixed seven-day Hobby sessions, one-day logs, branding, provider-managed signing keys, beta native prebuilt components and paid custom-session/MFA boundary are accepted. AUTH-12 stays fail-closed until Phase 1 creates and tests the separate production instance; AUTH-16 remains a pre-launch real-Philippines gate. The Worker keeps explicit bearer tokens, provider-neutral RS256/JWKS verification and `(issuer, subject)` mapping; no Clerk SDK or production credential enters the Phase 0B codebase. Self-hosted Better Auth remains rejected because it adds owned security/runtime complexity without a demonstrated requirement. |
...
| **ADR-134** | Dev Worker serves the mock-issuer JWKS at a dev-only route; production deploys from 0A with a no-issuer auth config | Accepted; amended | Production deploys from Phase 0A with no issuer configured at all: **every authenticated request in production returns 401 until Phase 0B selects the real provider** — this is intended, and it makes the ADR-132 failing-request acceptance test real. Production configuration contains neither the JWKS route nor any mock audience, JWKS URL or key (ADR-132). *Amended at implementation:* the original text had dev verification fetch the Worker's own JWKS route; a Worker fetching its own hostname is same-worker subrequest behavior we avoid rather than assume (§2). Instead the verifier reads mock public keys inline from the dev-only `MOCK_JWKS` variable, production environment validation **structurally rejects** `MOCK_JWKS`, the dev-only JWKS route remains for inspection, and the `AUTH_JWKS_URL` fetch+cache path — the real-provider path decided in 0B — is fully implemented and covered by the rejection-matrix tests. |
| **ADR-135** | ESLint + Prettier as lint and format tooling | Accepted | Enforced in CI from Phase 0A (the brief mandates lint and format checks but named no tools). The boring, heavily documented choice; typescript-eslint fits the strict central TypeScript configuration. Biome reconsidered only if tooling speed becomes a measured problem. |
| **ADR-136** | Worker binding types are generated by `wrangler types`, never hand-written | Accepted | `apps/api/worker-configuration.d.ts` (committed; regenerated explicitly by `pnpm types`; verified without mutation by `wrangler types --check` during typecheck) supplies the `Env` interface plus runtime types pinned to the configured compatibility date, replacing both the hand-written binding type and the `@cloudflare/workers-types` dependency. Generated with `--strict-vars=false`: vars stay `string`-typed because one codebase serves both environments; runtime Zod validation (`src/env.ts`) owns correctness. |
| **ADR-137** | Exact-origin CORS configured per environment | Accepted | The browser and API use separate local/deployed origins, so the Worker must opt into browser access. Each environment may name one validated `WEB_ORIGIN`; development requires it and production leaves it absent until a production web host exists. `/v1` allows only that exact origin, the current methods and headers, and exposes `X-Request-Id`. Wildcards and credentialed CORS are forbidden; bearer tokens remain explicit `Authorization` headers. ADR-127's Clerk selection keeps this boundary: platform SDKs obtain bearer tokens, and the API does not add provider cookies or another trusted origin. |
...
| **ADR-142** | Portfolio evidence is continuous; the public showcase is post-MVP and production-isolated | Accepted | Capture sanitized decision, design, implementation, validation and outcome evidence as each phase happens; reconstructing the reasoning at the end would lose the strongest case-study material. Evidence must use synthetic or redacted data and exclude secrets, provider identifiers, signed URLs, private user data, local media names and unlicensed media. A runnable public showcase remains deferred until after Phase 6 and production sign-off. It reuses the production web UI and contracts but is separately built and deployed with deterministic synthetic data and self-produced or openly licensed media. It has no production Clerk instance, API origin, D1 database, Bunny library, secrets, user data, upload path or administrative capability. Sensitive and cost-bearing flows use explicit adapters rather than runtime production conditionals. A dedicated brief and threat review must define the build boundary, licenses, reset behavior and CI isolation before showcase code begins. |
...
- Clerk Pro, a dedicated email provider, Sentry or PostHog until a required feature, measured deliverability issue or real allowance is exceeded.
- 4K outputs.
...
| **Post-MVP · Portfolio showcase** | Separate public build of the production web experience using synthetic data, licensed sample media and simulated sensitive flows; no production services or data | "A reviewer can explore the real experience without reaching the private product" | |
...
it is documentation work, not another product phase. The public showcase is
not a third Laya runtime environment and does not extend the product's
dev/production trust boundary from ADR-132. It is a separate public artifact
created only after Phase 6 and production sign-off. Its implementation starts
with a dedicated brief; it must reuse production UI components without copying
them into a long-lived fork, and its build must be provably incapable of
addressing production services. A written case study or recorded walkthrough
may ship earlier after a sanitization and licensing review because neither
changes application architecture nor exposes a runnable surface.
...
14. ~~**Public portfolio demonstration**~~ **Decided in ADR-142:** capture sanitized case-study evidence continuously, but build a production-isolated public showcase only after Phase 6 and production sign-off. Before implementation, its brief must choose and verify a build boundary that reuses the real web UI while making production API, identity, database, video and secret access structurally impossible.
...
| v1.3.1 | Jul 22, 2026 | Phase 0A implementation start: ADR-134 (dev Worker serves the mock-issuer JWKS at a dev-only route; production deploys from 0A with a no-issuer config, so all authenticated production requests 401 until 0B) and ADR-135 (ESLint + Prettier) added; §3.3 retitled "API module boundaries and conventions" to match its content. |
| v1.3.2 | Jul 22, 2026 | ADR-134 amended during implementation: dev verification reads mock keys from the dev-only `MOCK_JWKS` variable instead of self-fetching the Worker's own JWKS route (avoids relying on unverified same-worker subrequest behavior, §2); production env validation structurally rejects `MOCK_JWKS`; the JWKS-URL fetch+cache path remains the tested real-provider path for 0B. |
...
| v1.3.14 | Aug 8, 2026 | ADR-142 makes sanitized portfolio evidence a continuous practice while deferring the runnable public showcase until after Phase 6 and production sign-off; the future showcase must reuse the real web UI through a separately built synthetic-data artifact that is structurally disconnected from production. ADR-143 closes the Phase 1 tenancy gates with one shared MVP library and one viewer profile per account while preserving explicit library-membership and provider-neutral identity boundaries. DESIGN.md v0.3.3 and the Phase 1 brief add bounded evidence-capture guidance without expanding the product milestone. |
