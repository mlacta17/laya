...
| Add `en-sdh` candidate | HTTP 403 | Rejected: `srclang` permits only 1–3 lowercase letters | Not applicable | Not applicable | `sdh` was then accepted as a provisional spike key with label `English SDH`, version 1 |
| Add `en-forced` candidate | HTTP 403 | Rejected by the same 1–3-letter rule | Not applicable | Not applicable | `frc` was then accepted as a provisional spike key with label `English Forced`, version 1 |
| Verify three variants | HTTP 200 for `sdh` and `frc` | API exposes `en`, `sdh`, and `frc` simultaneously | Player exposes three distinct labels after refresh | Not separately sampled | Standard, SDH, and Forced rendered as distinct selectable tracks; final provider-key mapping still requires ADR-122 |
