# Laya — agent instructions

This is the single canonical rules file for every AI coding agent. Codex and other agents.md-aware tools read it directly; Claude Code loads it through the import in `CLAUDE.md`. Edit this file, not CLAUDE.md.
...
3. The active phase brief defines current scope: read `docs/phases/active/PHASE-0A-BRIEF.md` before starting any task.
...
- When a phase completes: date its row in ARCHITECTURE.md §12, fold learnings into ADRs, move the brief to `docs/phases/completed/` with its status block, then point at the next brief in two places — the path in §Authority above and the import in `CLAUDE.md`.
